...
(* token info record : id, option data, writer address, json file link *)
type token_info is record [
    id : token_id;
    option_data : option_info;
    link : map(string, bytes);
    issuer : address;
]
...
(* write option *)
function writeOption(const addr : address; option_params : option_info; var s : collection_storage) : return is
block {
    if Tezos.amount =/= size failwith("incorrect option size");
    const new_id : nat = Set.size(s.token_metadata); // id of the new token
    s.ledger[new_id] := address; // update ledger with new token
    ...
    s.token_metadata[new_id] := new_info; // update token metadata map
}
